...
#define BRIDGELENGTH 20

int addbridge(int l,int old){
  int i;
  for(i=0;i<6;i++){
    if(l>(int)(old/pow(BRIDGELENGTH,i))%BRIDGELENGTH)
      return BRIDGELENGTH*(old-old%(int)pow(BRIDGELENGTH,i)) + l*pow(BRIDGELENGTH,i) + old%(int)pow(BRIDGELENGTH,i);
  }
...
int getbridgefromSA(int big, int n){
  return (big/(int)pow(BRIDGELENGTH,n))%BRIDGELENGTH;
}
...
/*     printf("i=%i length=%i (*T).bridge[i].site[0]=%i,(*T).bridge[i].site[l]=%i\n",i,(*T).bridge[i].length,(*T).bridge[i].site[0],(*T).bridge[i].site[l]); */
    if(l==(*T).bridge[i].length){
      /* use the bridge.site[l+1], 0 for unlabeled, 1 for labeled */
      if((*T).bridge[i].site[l+1]==0){
...
int nextblockplist(int **plist,int *bp,const int *block,int nblock){
  /* nextblockplist gets the next plist from bp */
  int i;
  if(*bp<0)
    return 0;
  if(nextplist(plist[*bp],block[*bp])==0){
    /* init plist bp*/
    for(i=0;i<block[*bp];i++)
      plist[*bp][i]=i;
    /* and decrements bp */
    if(*bp==0)
      return 0;
      ...
      (*bp)--;
      return nextblockplist(plist,bp,block,nblock);
    }
  }
  else{
    /* successfully incremented bp */
    /* set bp to nblock-1 */
    *bp=nblock-1;
    return 1;
  }
...
int displaySA2(int SA[MAXSITE][MAXSITE+1],int Nnode){
  int j,k;
  for(k=0;k<Nnode;k++){
    for(j=0;j<Nnode+1;j++){
      printf("%7i ",SA[k][j]);      
...
void displayplist(int **plist,int nblock, int *block){
  int j,k;
  for(j=0;j<nblock;j++){
    for(k=0;k<block[j];k++){  
      printf("%i ",plist[j][k]);
...
int ordernodes(int SA[MAXSITE][MAXSITE+1],int Nnode,int *sitemap){
  int sitemapnew[MAXSITE],sitemapold[MAXSITE];
  int i,j,k,l,done;
  int valence[MAXSITE];
  int block[MAXSITE];
  int boffset;
  int nblock,curval,node;
  int **plist;
  int bp[1];
  int blockplist[MAXSITE];
  int SAnew[MAXSITE][MAXSITE+1],SAold[MAXSITE][MAXSITE+1];

  /* this is not the actual valency, since multiple bridges */
  /* to the same node are counted as one */
  for(k=0;k<Nnode;k++){
    valence[k]=0;
    for(l=0;l<Nnode+1;l++){
      if(SA[k][l]!=0){
...
/*     printf("node %i valence %i\n",k,valence[k]); */
  }

  /* go through nodes, putting the lowest valency first */
  nblock=0;
  node=0;
  for(curval=1;curval<=COORD;curval++){
    block[nblock]=0;   
    for(k=0;k<Nnode;k++){
      if(valence[k]==curval){
	blockplist[k]=node;
  ...
  for(k=0;k<Nnode;k++){
    for(l=0;l<Nnode;l++){
      SAold[blockplist[k]][blockplist[l]]=SA[k][l];
    }
    SAold[blockplist[k]][Nnode]=SA[k][Nnode]; 
    sitemapold[blockplist[k]]=sitemap[k];
  }

/*   printf("SA\n"); */
/*   displaySA2(SA,Nnode); */
/*   for(k=0;k<Nnode;k++){ */
/*     printf("%i ",blockplist[k]); */
/*   } */
/*   printf("\n"); */
/*   printf("SA block\n"); */
/*   displaySA2(SAold,Nnode); */
/*   for(i=0;i<nblock;i++) */
/*     printf("block[%i]=%i\n",i,block[i]); */


  /* copy SAold into the original*/
  for(k=0;k<Nnode;k++){
    for(l=0;l<Nnode+1;l++){
      SA[k][l]=SAold[k][l];
    }
    sitemap[k]=sitemapold[k];
  }
  
  plist = (int **)malloc(sizeof(int *)*nblock);

  /* init plist*/
  for(j=0;j<nblock;j++){
    plist[j] = (int *)malloc(sizeof(int)*block[j]);
    for(k=0;k<block[j];k++){      
      plist[j][k]=k;      
    }
  }
...
/*   printf("plist ,nblock = %i :",nblock); */
/*   displayplist(plist,nblock,block); */

  /* build a block list */
  while(nextblockplist(plist,bp,block,nblock)){
    l=0;
    boffset=0;
    for(j=0;j<nblock;j++){
      for(k=0;k<block[j];k++){      
	blockplist[l]=boffset+plist[j][k];
	l++;
      }
      boffset+=block[j];
    }
    ...
    for(k=0;k<Nnode;k++){
      for(l=0;l<Nnode;l++){
	SAnew[blockplist[k]][blockplist[l]]=SAold[k][l];
      }
      SAnew[blockplist[k]][Nnode]=SAold[k][Nnode];
      sitemapnew[blockplist[k]]=sitemapold[k];
    }
/*     printf("SA trial\n"); */
/*     displaySA2(SAnew,Nnode); */
...
topo topology(bondgraphbig B){
  ...
  if(T.Nnode==0){
	...
	if(valence[sitevar]==1){
	  ...
	  if(newbridge){
	    SAtemp[j][j]= addbridge(T.bridge[n].length,SAtemp[node[sitevar]][node[sitevar]]);
	    n++;
	  }
	}
	else{
...
/* 	  printf("Bridge %i,length %i:\t",n,T.bridge[n].length); */
/* 	  for(l=0;l<=T.bridge[n].length;l++) */
/* 	    printf("%i ",T.bridge[n].site[l]); */
/* 	  printf("\n"); */
