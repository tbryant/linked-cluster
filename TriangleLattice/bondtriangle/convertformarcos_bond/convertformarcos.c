...
int main(int argc, char *argv[]){
  ...
  if(argc!=3){
    printf("convertformarcos.e Nbondmax inputlogfile\n");
    exit(0);
  }
...
/*     printf("%i %i %i %i\n",Nbond,Nsite,temp1,temp2); */
  ...
  /* open info file */
  sprintf(filename,"output%i/TOPOLOGY_INFORMATION.dat",Nbondmax);
  printf("writing %s\n",filename);
  ...
  /* open site info file */
  sprintf(filename,"output%i/SITENUMBER_INFORMATION.dat",Nbondmax);
  printf("writing %s\n",filename);
  if ((infofp = fopen(filename, "w"))==NULL){
    printf("Cannot open file %s for output\n",filename);
    exit(1);
  ...
  for(Nbond=1;Nbond<=Nbondmax;Nbond++){
    if(toposum[Nbond]!=0){
      ...
      sprintf(filename,"output%i/TOPO_GRAPH_%02i_BONDS.bin",Nbondmax,Nbond);
      printf("writing %s\n",filename);
      if ((mtopofp = fopen(filename, "w"))==NULL){
	printf("Cannot open file %s\n",filename);
	exit(1);
      }
      sprintf(filename,"output%i/TOPO_MULT_%02i_BONDS.bin",Nbondmax,Nbond);
      printf("writing %s\n",filename);
	...
	exit(1);
      ...
      for(Nsite=2;Nsite<MAXSITE;Nsite++){
  ...
  for(Nbond=1;Nbond<=Nbondmax;Nbond++){
    if(toposum[Nbond]!=0){
      /* open output files for Nbond */
      sprintf(filename,"output%i/SUBGRAPH_LIST_%02i_BONDS.bin",Nbondmax,Nbond);
      printf("writing %s\n",filename);
      if ((mtopofp = fopen(filename, "w"))==NULL){
	printf("Cannot open file %s\n",filename);
	exit(1);
      }
      sprintf(filename,"output%i/SUBGRAPH_TOPOMULT_%02i_BONDS.bin",Nbondmax,Nbond);
      printf("writing %s\n",filename);
      if ((mmultfp = fopen(filename, "w"))==NULL){
	printf("Cannot open file %s\n",filename);
	exit(1);
